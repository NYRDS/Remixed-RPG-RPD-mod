...
return item.init{
    desc  = function (self, item)
      ...
      stats = a[2]
        return {
            ...
            name          = "Боевой молот: "..tostring(math.max(stra-item:level(),1)),
            price         = 20*2^(tier-1)+40*item:level(),
            stackable     = false,
            upgradable    = true,
            equipable     ="weapon"
        }
    ...
    info = function(self,item)
      ...
      str = stra-item:level()
      local info = "Очень немногие существа могут пережить удар такого оружия, но не каждому путешественнику под силу его использовать. \n\nБоевой молот - оружие ближнего боя 5 порядка. Средний урон составляет 14 единиц за удар и, как правило, требует "..stra.." очков силы. Это довольно меткое оружие.\n\n"..self.data.sInfo
    ...
    damageRoll = function(self, item, user)
        return 18+5*item:level(),9+5*item:level()
    end,
