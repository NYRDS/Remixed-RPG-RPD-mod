...
return item.init{
    desc  = function (self, item)
      ...
      stats = a[2]
        return {
            ...
            name          = "Булава: "..tostring(math.max(stra-item:level(),1)),
    ...
    info = function(self,item)
      ...
      str = stra-item:level()
      local info = "Железный наконечник этого оружия способен наносить существенный урон. \n\nБулава - оружие ближнего боя "..tier.." порядка. Средний урон составляет "..((maxDmg+minDmg+tier*item:level()*2)/2).." единиц за удар и, как правило, требует "..stra.." очков силы. Это довольно меткое оружие.\n\n"..self.data.sInfo
