...
return item.init{
    desc  = function (self, item)
      ...
      stats = a[2]
        return {
            ...
            name          = "Клеймор: "..tostring(math.max(stra-item:level(),1)),
    ...
    info = function(self,item)
      ...
      str = stra-item:level()
      local info = "Этот увесистый меч должен быть в состоянии разрубить всё что угодно. \n\nКлеймор - оружие ближнего боя"..tostring(tier).." порядка. Средний урон составляет"..tostring((maxDmg+minDmg+tier*item:level()*2)/2).." единиц за удар и, как правило, требует "..stra.." очков силы.\n\n"..self.data.sInfo
