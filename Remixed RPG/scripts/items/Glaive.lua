...
return item.init{
    desc  = function (self, item)
      ...
      stats = a[2]
        return {
            ...
            name          = "Глефа: "..tostring(math.max(stra-item:level(),1)),
            price         = 20*2^(tier-1)+50*item:level(),
            stackable     = false,
            upgradable    = true,
            equipable     ="weapon"
        }
    ...
    info = function(self,item)
      ...
      str = stra-item:level()
      local info = "Древковое оружие с лезвием от меча на конце древка.\n\nГлефа - оружие ближнего боя"..tostring(tier).." порядка. Средний урон составляет"..tostring((maxDmg+minDmg+tier*item:level()*2)/2).." единиц за удар и, как правило, требует "..stra.." очков силы. Это довольно медленное, но меткое оружие.\n\n"..self.data.sInfo
