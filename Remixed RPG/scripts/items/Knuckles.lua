...
return item.init{
    ...
    info = function(self,item)
      ...
      str = stra-item:level()
      local info = "Железная скоба, плотно охватывающая пальцы. \n\nКастет - оружие ближнего боя "..tier.." порядка. Средний урон составляет "..((maxDmg+minDmg+tier*item:level()*2)/2).." единиц за удар и, как правило, требует "..stra.." очков силы. Это довольно быстрое оружие.\n\n"..self.data.sInfo
